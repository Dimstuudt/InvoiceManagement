<template>
  <AppLayout title="Urutan Nomor Invoice">
    ...
    <Transition name="numbering-lock">
      <div v-if="!bypassActive"
        ...
        <div class="bg-white rounded-2xl shadow-2xl border border-gray-100 px-8 py-10 text-center max-w-sm w-full mx-4">
          ...
          <p class="text-sm text-gray-500 leading-relaxed mb-6">
            Urutan Nomor Invoice hanya bisa diakses saat <strong class="text-gray-700">Bypass Verifikasi</strong> aktif.<br>
    ...
    <div class="space-y-5 transition-[filter] duration-300"
...
<style scoped>
.numbering-lock-enter-active { transition: opacity 0.2s ease, transform 0.25s cubic-bezier(0.34, 1.56, 0.64, 1); }
.numbering-lock-leave-active { transition: opacity 0.15s ease; }
.numbering-lock-enter-from   { opacity: 0; transform: translateY(-8px) scale(0.97); }
.numbering-lock-leave-to     { opacity: 0; }
</style>
