<template>
  <AppLayout title="Activity Log">
    <div class="max-w-4xl mx-auto space-y-5">
      ...
      <!-- Filter Bar -->
      <div class="bg-white rounded-2xl border border-slate-200 shadow-sm px-4 py-3 flex flex-col sm:flex-row gap-3 items-center">
        <div class="relative flex-1 w-full">
          ...
          <input v-model="search" type="text"
            placeholder="Cari aktivitas, nama, IP..."
        ...
        <div v-for="[dateLabel, entries] in groupedLogs" :key="dateLabel">
          ...
          <div class="mt-3 space-y-2">
            <div v-for="log in entries" :key="log.id"
              class="bg-white rounded-xl border border-slate-100 shadow-sm hover:shadow hover:border-slate-200 transition-all duration-150 flex gap-3 px-4 py-3">
                ...
                <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] text-slate-400">
                  ...
                  <span v-if="log.user_agent" class="flex items-center gap-1 min-w-0">
                    ...
                    <span class="truncate max-w-xs" :title="log.user_agent">
                      {{ log.user_agent.length > 72 ? log.user_agent.substring(0, 72) + '…' : log.user_agent }}
                    </span>
                  </span>
                  <!-- Subject link -->
      ...
      <div v-else class="bg-white rounded-2xl border border-slate-200 shadow-sm py-20 text-center">
        ...
        <p class="mt-1 text-xs text-slate-400">Coba ubah kata kunci pencarian atau pilih kategori yang berbeda.</p>
        <button v-if="search || kategori" @click="search = ''; setKategori('')"
...
function describe(log) {
  ...
  switch (log.action) {
    ...
    case 'invoice.status_changed':    return `Status ${s} berubah dari ${d.from ?? '?'} → ${d.to ?? '?'}`
