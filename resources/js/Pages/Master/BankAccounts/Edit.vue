  ...
  <AppLayout title="Edit Bank Account">
    <div class="max-w-xl">
      <div class="bg-white rounded-2xl border border-gray-200 p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-5">Edit Bank Account</h2>
        <form @submit.prevent="submit" class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Nama Bank</label>
            <input v-model="form.bank_name" type="text" class="w-full px-4 py-2.5 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" :class="form.errors.bank_name ? 'border-red-400' : 'border-gray-300'" />
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Logo Bank</label>
...
function submit() {
  form.post(route('master.bank-accounts.update', props.account.id), { _method: 'put' });
}
</script>
