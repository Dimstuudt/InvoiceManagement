<template>
  <AppLayout title="Edit Client Category">
    <div class="max-w-xl mx-auto">
      <div class="bg-white rounded-2xl border border-gray-200 p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-5">Edit Client Category</h2>
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div class="flex gap-3 pt-2">
            ...
            <Link :href="route('master.client-categories.index')" class="px-5 py-2.5 text-sm font-medium text-gray-600 hover:text-gray-800 border border-gray-300 hover:border-gray-400 rounded-lg transition-colors">Batal</Link>
          </div>
        </form>
      </div>
    </div>
...
function submit() { form.put(route('master.client-categories.update', props.category.id)); }
</script>
