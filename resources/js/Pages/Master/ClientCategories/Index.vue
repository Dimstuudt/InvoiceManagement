  ...
  <AppLayout title="Client Categories">
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-lg font-semibold text-gray-800">Client Categories</h2>
          <p class="text-sm text-gray-500">{{ categories.length }} kategori</p>
        </div>
        <Link :href="route('master.client-categories.create')" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg transition-colors">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-if="categories.length === 0">
              <td colspan="3" class="px-6 py-10 text-center text-sm text-gray-400">Belum ada data.</td>
            </tr>
            <tr v-for="cat in categories" :key="cat.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 text-right space-x-2">
                <Link :href="route('master.client-categories.edit', cat.id)" class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-800 border border-indigo-200 hover:border-indigo-400 rounded-lg transition-colors">Edit</Link>
