    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <TrashPanel type="client-categories" :items="trashed">
            <template #info-header>Deskripsi</template>
            <template #info="{ item }">{{ item.description ?? '—' }}</template>
          </TrashPanel>
...
async function goEdit(id) {
  ...
  router.visit(route('master.client-categories.edit', id));
}
...
async function bulkDestroy() {
  if (!await requireGate()) return;
  router.delete(route('master.client-categories.bulk-destroy'), {
