<template>
  <AppLayout title="Buat Grup Template Email">
      ...
      <div class="w-full lg:w-[480px] lg:shrink-0 bg-white border-b lg:border-b-0 lg:border-r border-gray-200 overflow-y-auto">
        <div class="p-6">
          ...
          <div class="flex items-center gap-2 mb-6">
            ...
            <h2 class="text-base font-semibold text-gray-800">Grup Template Baru</h2>
          </div>

          <!-- Banner referensi -->
          <div v-if="isPrefill" class="mb-5 flex items-start gap-3 px-4 py-3 bg-indigo-50 border border-indigo-200 rounded-xl">
            ...
            <div>
              <p class="text-sm font-semibold text-indigo-700">Sudah diisi otomatis ✦</p>
              <p class="text-xs text-indigo-500 mt-0.5">Semua tahap sudah ada contoh teksnya. Ubah sesuka kamu sebelum disimpan — tidak ada yang salah di sini.</p>
            </div>
          </div>
          ...
          <form @submit.prevent="submit" class="space-y-6">
            ...
            <div>
              ...
              <input v-model="form.name" type="text" placeholder="misal: Template Tagihan Bulanan"
              ...
              <div v-for="stage in STAGES" :key="stage.key" v-show="activeStage === stage.key" class="space-y-3">
                ...
                <div>
                  <div class="flex items-center justify-between mb-1">
                    <label class="block text-xs font-medium text-gray-500">Isi Email (Body)</label>
                    <span class="text-[10px] text-gray-400">klik variabel di bawah untuk copy</span>
                  </div>
            ...
            <div class="flex gap-3 pt-2 pb-8">
              <button type="submit" :disabled="form.processing"
                ...
                {{ form.processing ? 'Menyimpan...' : 'Simpan Grup' }}
              </button>
              <Link :href="route('master.email-template-groups.index')"
...
const DEFAULT_TEMPLATES = {
  send1: {
    subject: 'Invoice {{invoice_number}} — {{project_name}} | {{client_name}}',
    body:
`Yth. {{director}},

Salam hormat,

Bersama email ini kami kirimkan invoice dengan rincian berikut:
...
Mohon melakukan pembayaran sebelum tanggal {{due_date}} ke rekening berikut:

Bank         : {{bank_name}}
No. Rekening : {{bank_account_number}}
Atas Nama    : {{bank_holder}}

Terlampir file PDF invoice untuk referensi. Apabila ada pertanyaan, silakan hubungi kami.
...
{{issuer_name}}`,
  ...
  send2: {
    subject: '[Pengingat] Invoice {{invoice_number}} — Jatuh Tempo {{due_date}}',
    body:
`Yth. {{director}},

Kami ingin mengingatkan bahwa invoice berikut belum kami terima pembayarannya:
...
{{issuer_name}}`,
  ...
  send3: {
    subject: '[Pengingat ke-2] Invoice {{invoice_number}} — Segera Lunasi',
    body:
`Yth. {{director}},

Ini adalah pengingat kedua untuk invoice yang belum dibayarkan:

No. Invoice  : {{invoice_number}}
Jatuh Tempo  : {{due_date}}
Total        : {{amount}}

Kami mohon agar pembayaran segera dilakukan. Keterlambatan dapat mempengaruhi kelanjutan layanan.
...
{{issuer_name}}`,
  ...
  send4: {
    subject: '[MENDESAK] Invoice {{invoice_number}} — Pembayaran Segera Diperlukan',
    body:
`Yth. {{director}},

Tagihan Anda sudah melewati jatuh tempo. Hingga saat ini kami belum menerima pembayaran untuk:

No. Invoice  : {{invoice_number}}
Jatuh Tempo  : {{due_date}}
Total        : {{amount}}

Kami mohon dengan sangat agar pembayaran segera diselesaikan. Jika ada kendala, hubungi kami sesegera mungkin.
...
{{issuer_name}}`,
  ...
  send5: {
    subject: '[PERHATIAN] Invoice {{invoice_number}} — Layanan Akan Dinonaktifkan',
    body:
`Yth. {{director}},

PERHATIAN: Tagihan ini belum dibayarkan hingga batas waktu yang ditentukan. Layanan akan segera dinonaktifkan jika pembayaran tidak segera dilakukan.

No. Invoice  : {{invoice_number}}
Jatuh Tempo  : {{due_date}}
Total        : {{amount}}

Segera lakukan pembayaran atau hubungi kami untuk menjadwalkan penyelesaian:
...
{{issuer_name}}`,
  ...
  paid: {
    subject: 'Konfirmasi Pembayaran Invoice {{invoice_number}} — Terima Kasih',
    body:
`Yth. {{director}},

Terima kasih! Kami telah menerima pembayaran untuk invoice berikut:

No. Invoice  : {{invoice_number}}
Proyek       : {{project_name}}
Total        : {{amount}}

Terlampir bukti penerimaan pembayaran (receipt) untuk arsip Anda.

Kami menghargai kepercayaan {{client_name}} kepada kami dan berharap dapat terus melayani Anda dengan baik.
...
const isPrefill = new URLSearchParams(window.location.search).get('prefill') === 'default';

const emptyStages = Object.fromEntries(
  STAGES.flatMap(s => [[`subject_${s.key}`, ''], [`body_${s.key}`, '']])
);

const prefillData = isPrefill
  ? Object.fromEntries(
      STAGES.flatMap(s => [
        [`subject_${s.key}`, DEFAULT_TEMPLATES[s.key].subject],
        [`body_${s.key}`,    DEFAULT_TEMPLATES[s.key].body],
      ])
    )
  : emptyStages;

const form = useForm({
  name:       isPrefill ? 'Template Tagihan Standar' : '',
  is_default: false,
  ...prefillData,
});
...
function submit() {
  form.post(route('master.email-template-groups.store'));
}
</script>
