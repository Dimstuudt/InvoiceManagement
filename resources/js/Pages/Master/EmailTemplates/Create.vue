<template>
  <AppLayout title="Tambah Template Email">

    <!-- Split-pane editor: full height, edge-to-edge, masing-masing scroll sendiri -->
      ...
      <div class="flex-1 bg-white border-b lg:border-b-0 lg:border-r border-gray-200 overflow-y-auto lg:min-w-0">
        <div class="p-6 max-w-xl">
          ...
          <div class="flex items-center gap-2 mb-6">
            ...
            <h2 class="text-base font-semibold text-gray-800">Template Email Baru</h2>
          </div>
          ...
          <div v-if="isPrefill" class="mb-5 flex items-start gap-3 px-4 py-3 bg-indigo-50 border border-indigo-200 rounded-xl">
            ...
            <div>
              <p class="text-sm font-semibold text-indigo-700">Template Referensi</p>
              <p class="text-xs text-indigo-500 mt-0.5">Form diisi dengan template standar sistem. Sesuaikan sebelum menyimpan.</p>
            </div>
          </div>
          ...
          <form @submit.prevent="submit" class="space-y-5">
            ...
            <div>
              ...
              <input v-model="form.subject" type="text" placeholder="misal: Invoice {{invoice_number}} - {{client_name}}"
                class="w-full px-4 py-2.5 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition"
            ...
            <div>
              ...
              <textarea v-model="form.body" rows="16"
                placeholder="Yth. {{director}},&#10;&#10;Bersama email ini kami kirimkan invoice {{invoice_number}}..."
...
Bersama email ini kami kirimkan invoice dengan rincian sebagai berikut:
...
Terlampir file PDF invoice untuk referensi. Apabila ada pertanyaan, jangan hesitasi untuk menghubungi kami.
...
const form = useForm(isPrefill ? { ...DEFAULT_TEMPLATE } : {
...
function submit() {
  form.post(route('master.email-templates.store'));
}
</script>
