  ...
  <AppLayout title="Edit Signature">
    <div class="max-w-xl">
      <div class="bg-white rounded-2xl border border-gray-200 p-6">
        <h2 class="text-base font-semibold text-gray-800 mb-5">Edit Signature</h2>
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <div>
            ...
            <input v-model="form.position" type="text" class="w-full px-4 py-2.5 rounded-lg border text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 transition" :class="form.errors.position ? 'border-red-400' : 'border-gray-300'" />
            <p v-if="form.errors.position" class="mt-1 text-xs text-red-500">{{ form.errors.position }}</p>
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1.5">Gambar Tanda Tangan</label>
...
function submit() {
  form.post(route('master.signatures.update', props.signature.id), { _method: 'put' });
}
</script>
